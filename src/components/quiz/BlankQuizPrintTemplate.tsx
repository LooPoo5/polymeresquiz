import React, { forwardRef } from 'react';
import { Question } from '@/context/QuizContext';

interface BlankQuizPrintTemplateProps {
  title: string;
  imageUrl?: string;
  questions: Question[];
}

const BlankQuizPrintTemplate = forwardRef<HTMLDivElement, BlankQuizPrintTemplateProps>(
  ({ title, imageUrl, questions }, ref) => {
    return (
      <div ref={ref} className="bg-white p-8 text-black">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-4 text-center">{title}</h1>
          {imageUrl && (
            <div className="flex justify-center mb-6">
              <img 
                src={imageUrl} 
                alt="Quiz" 
                className="max-w-full h-auto max-h-48 object-contain"
              />
            </div>
          )}
        </div>

        {/* Participant Information Section */}
        <div className="mb-8 p-4 border-2 border-gray-300 rounded">
          <h2 className="text-xl font-bold mb-4">Informations du participant</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold mb-2">Nom du participant :</label>
              <div className="border-b-2 border-gray-400 h-8"></div>
            </div>
            <div>
              <label className="block text-sm font-semibold mb-2">Date :</label>
              <div className="border-b-2 border-gray-400 h-8"></div>
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-semibold mb-2">Formateur :</label>
              <div className="border-b-2 border-gray-400 h-8"></div>
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-semibold mb-2">Signature :</label>
              <div className="border-b-2 border-gray-400 h-20"></div>
            </div>
          </div>
        </div>

        {/* Questions */}
        <div className="space-y-6">
          {questions.length === 0 ? (
            <p className="text-gray-500 text-center py-8">Aucune question disponible</p>
          ) : (
            questions.map((question, index) => (
              <div key={question.id} className="break-inside-avoid mb-6 page-break-inside-avoid">
                <div className="mb-3">
                  <h3 className="text-lg font-bold mb-2">
                    Question {index + 1}: {question.text}
                  </h3>
                  {question.imageUrl && (
                    <div className="my-3">
                      <img 
                        src={question.imageUrl} 
                        alt={`Question ${index + 1}`}
                        className="max-w-full h-auto max-h-40 object-contain"
                      />
                    </div>
                  )}
                </div>

                {question.type === 'multiple-choice' && (
                  <div className="space-y-2 ml-4">
                    {question.answers.map((answer) => (
                      <div key={answer.id} className="flex items-center gap-3">
                        <div className="w-5 h-5 border-2 border-gray-400 rounded flex-shrink-0"></div>
                        <span className="text-sm">{answer.text}</span>
                      </div>
                    ))}
                  </div>
                )}

                {question.type === 'checkbox' && (
                  <div className="space-y-2 ml-4">
                    {question.answers.map((answer) => (
                      <div key={answer.id} className="flex items-center gap-3">
                        <div className="w-5 h-5 border-2 border-gray-400 flex-shrink-0"></div>
                        <span className="text-sm">{answer.text}</span>
                      </div>
                    ))}
                  </div>
                )}

                {question.type === 'open-ended' && (
                  <div className="ml-4 mt-3">
                    <div className="border-2 border-gray-300 rounded p-3 min-h-[100px]">
                      <p className="text-xs text-gray-500 italic">Espace pour la réponse...</p>
                    </div>
                  </div>
                )}
              </div>
            ))
          )}
        </div>
      </div>
    );
  }
);

BlankQuizPrintTemplate.displayName = 'BlankQuizPrintTemplate';

export default BlankQuizPrintTemplate;
